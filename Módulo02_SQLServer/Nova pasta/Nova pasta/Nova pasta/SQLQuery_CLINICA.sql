...
(
   ...
   Categoria int not null CHECK (Categoria = 1 AND Categoria = 2)
)
...
(
   ...
   Duracao int not null CHECK (Duracao >=1 and Duracao<=5),
...
(
   ...
   FOREIGN KEY (Id_Unidade) REFERENCES TB_Materiais_e_Medicamentos(Id),
   FOREIGN KEY (Id_Procedimento) REFERENCES TB_Procedimentos (Id)
   )